...
void finalDecrypt(TLweSample *ciphertext, TorusPolynomial **partial_ciphertexts, TLweParams *params, uint64_t *cycle_counts, int party, std::vector<int> parties, int t, int p)
{
	...
	int threshold = t;
	int totalParties = p;
	int partyId = parties[party];
	std::vector<std::string> candidates = {"Captain America", "Iron Man", "Thor"};
	candidates;
	double time = (((double)end_time.tv_nsec + 1.0e+9 * end_time.tv_sec) - ((double)start_time.tv_nsec + 1.0e+9 * start_time.tv_sec)) * 1.0e-9;
	std::cout << threshold << "-out-of-" << totalParties << " | Official " << partyId << " | Result: " << candidates[result_msg] << " | Time: " << time << "s\n";
}